...
class IoULoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        for i in range(0, b):
            ...
            eps = 1e-6 # as a patch can have no foreground
            Iand1 = torch.sum(target[i, :, :, :] * pred[i, :, :, :])
            Ior1 = torch.sum(target[i, :, :, :]) + torch.sum(pred[i, :, :, :]) - Iand1
            IoU1 = (Iand1+eps) / (Ior1+eps) # stability
            # IoU loss is (1-IoU1)
            IoU = IoU + (1-IoU1)
        # return IoU/b
        return IoU
...
class PatchIoULoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        if len(target.shape) != 4:
            raise RuntimeError(f'Expected target shape [B, C, H, W], got {target.shape}')

        win_y, win_x = 64, 64
        iou_loss = 0.
        for anchor_y in range(0, target.shape[2], win_y):
            for anchor_x in range(0, target.shape[3], win_x):
